...
use SerBinario\MBCredito\NewCBOBundle\Util\GridClassWithLimit;
...
class CBGController extends Controller
{
    ...
    public function gridTransacoesCBGAction(Request $request)
    {
        if(GridClassWithLimit::isAjax()) {
            ...
            $whereValueMain   = "";
            $whereFull        = "";
            $groupBy          = "a.chaveOperador";

            $gridClass = new GridClassWithLimit($this->getDoctrine()->getManager(),
                $parametros,
                $columns,
                $entity,
                $entityJOIN,
            ...
            $countTotal      = $gridClass->getCountGroupBy($groupBy);
            $countDetalhe    = count($resultDetalhe);

            for($i=0;$i < $countDetalhe; $i++)
            {
                $detalheArray[$i]['DT_RowId']  = "row_". $i;
                $detalheArray[$i]['chave']     =  $resultDetalhe[$i]['chaveOperador'];
                $detalheArray[$i]['valor']     =  number_format($resultDetalhe[$i]['valorParcela'], 2, ',', '.');
            }
