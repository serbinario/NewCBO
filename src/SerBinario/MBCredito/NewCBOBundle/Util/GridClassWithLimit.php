...
class GridClassWithLimit
{
    ...
    {        
        try {
            ...
            $query  = $this->em->createQuery("SELECT distinct a.chaveOperador, sum(a.valorParcela) as valorParcela FROM {$this->entity} a"
                        . " {$sqlJoin} "
                        . " {$dqlWhere} "                      
                        . " {$groupBy} ORDER BY {$dqlOrder}")
     ...
     * @return int|mixed
     */
    public function getCountGroupBy($groupBy)
    {
        try {
            $qb = $this->em->createQueryBuilder();
            $qb->select("count(a)");
